...
# detect only Red and Yellow colors for the traffic lights (need to stop as detecting them)
...
# detect TL objects and crop this part to recognize color inside
#  create a stop flag to take the actions based on recognized color of the traffic light.
...
def plot_origin_image(image_np, boxes, classes, scores, category_index, idx):
    ...
    plt.savefig( 'outputs/tl_detection/out_' + str(idx) +'.png')
    # plt.show()
...
def detect_traffic_lights(PATH_TO_TEST_IMAGES_DIR, MODEL_NAME, Num_images, plot_flag=False):
    ...
    PATH_TO_LABELS = 'scripts/tl_detection/mscoco_label_map.pbtxt'
    ...
    with detection_graph.as_default():
        with tf.compat.v1.Session(graph=detection_graph) as sess:
            ...
            for idx, image_path in enumerate(TEST_IMAGE_PATHS):
                ...
                if plot_flag:
                    plot_origin_image(image_np, boxes, classes, scores, category_index, idx)
